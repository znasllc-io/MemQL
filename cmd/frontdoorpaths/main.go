...
// renderGo turns the same path list into the committed Go artifact
// component/frontdoor/paths.generated.go (epic memql#5168).
//
// # WHY A SECOND OUTPUT RATHER THAN A SECOND LIST
//
// An account's reserved front door serves `api.<reserved>`, and design D5 says
// it routes byte-identically to the cluster's own `api.` host. That routing is
// applied at RUNTIME by a capability script, not by a rendered overlay -- so
// something in Go has to know the path list, and the only honest way for it to
// know is to be handed the output of this same collect().
//
// The alternative considered and rejected was a hand-kept subset of "what a
// client actually calls" (the catch-all, /memql/ws, /artifacts*, /healthz).
// It is smaller and reads better in a rendered Ingress, and it is exactly the
// second source this repository keeps paying for: the next HTTP route added to
// the bff would be silently unrouted under every client's name, and would fail
// as an HTTP/1.1 request handed to an h2c backend -- a protocol error naming
// no path, no host and no generator.
//
// It lands in component/frontdoor because that module has no dependencies and
// a []string adds none, and because the manifest generator and the account
// provisioner then read ONE artifact.
//
// The ORDER is carried over verbatim and matters for the reason collect()
// gives: longest-first makes traefik's specificity ordering and nginx's
// declaration ordering agree without relying on either one's tie-breaking, and
// the trailing-slash collapse above it is what stops nginx refusing the whole
// config with a duplicate-location error.
func renderGo(paths []string) string {
	var b strings.Builder
	b.WriteString("// Code generated by cmd/frontdoorpaths. DO NOT EDIT.\n")
	b.WriteString("// Regenerate with: make frontdoor\n\n")
	b.WriteString("package frontdoor\n\n")
	b.WriteString("// BFFHTTPPaths is every HTTP path the front door routes to the bff's HTTP\n")
	b.WriteString("// edge (bff-http:8085) rather than to its gRPC h2c edge, longest-first.\n")
	b.WriteString("//\n")
	b.WriteString("// Generated from the same collect() that writes the Ingress path blocks in\n")
	b.WriteString("// deploy/k8s/overlays, so the cluster's own `api.` host and every account's\n")
	b.WriteString("// reserved `api.` host route the same set by construction. Read by the\n")
	b.WriteString("// account front-door provisioner, which hands it to\n")
	b.WriteString("// scripts/deploy/bind-account-front-door.sh.\n")
	b.WriteString("//\n")
	b.WriteString("// The catch-all \"/\" is deliberately ABSENT: it is the rule that reaches the\n")
	b.WriteString("// gRPC backend, and emitting it here would shadow that backend with the\n")
	b.WriteString("// HTTP one.\n")
	b.WriteString("var BFFHTTPPaths = []string{\n")
	for _, p := range paths {
		fmt.Fprintf(&b, "\t%q,\n", p)
	}
	b.WriteString("}\n")
...
func main() {
	...
	emitGo := flag.String("emit-go", "", "write the path list as the committed Go artifact at this path")
	flag.Parse()

	paths := collect()

	// --emit-go is its own mode rather than a side effect of --write: the Go
	// artifact is written ONCE per `make frontdoor`, while --write runs per
	// overlay file, and writing it three times would make the last overlay in
	// the Makefile's list silently responsible for it.
	if *emitGo != "" {
		if err := os.WriteFile(*emitGo, []byte(renderGo(paths)), 0o644); err != nil {
