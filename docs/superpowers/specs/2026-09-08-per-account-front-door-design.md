...
| D9a | A live door's DNS | **Re-checked, and demoted on sustained drift.** ADDED after review: the original design re-checked pointing only on the way to `live` and never again, so a door whose `app.` host was repointed afterwards kept serving AND kept its callback registered as an OAuth redirect URI for the OS client. That composes: repoint the host at a server you control, send somebody a sign-in link at this cluster naming that callback, and the authorization code arrives at your server through a consent page carrying this platform's own name. A redirect allowlist that can go stale is not a control. Live doors are re-checked every 15 minutes and demoted to `verifying` after 3 consecutive failures -- which stops the edge resolving the host and drops the door out of the identity node's live set in the same write. A counter rather than an immediate demotion because the blast radius of a false positive is every one of a client's people |
