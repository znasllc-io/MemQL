// Keeping a vendor API key out of the install receipt and the run log.
//
// WHY THIS EXISTS. The install used to collect `providerKeyFile` -- "A PATH to
// a file holding the key, never the key itself", as the field's own hint said
// -- and for a long time that sentence was the entire enforcement. An operator
// who pasted their key instead got it accepted, handed to the capability script
// as `--key-file=sk-ant-...` (argv, which `ps` shows to every process on the
// machine) and then written into the install receipt, where it stayed in
// plaintext for the life of the install.
//
// IT HAD TWO CALLERS, AND ONLY ONE IS LEFT (epic memql#5088).
// `state/addCluster.ts` refused the value where it was typed, which is the one
// place a person can be told what to do instead; that was the FIX, and it went
// with the field. `install/receipt.ts` redacts on the WRITE, which covers every
// OTHER way a param can reach the receipt -- the CLI, a graph-pinned param, a
// surface nobody has written yet -- and `state/runLog.ts` does the same on the
// way into a run log. That is the WALL, and it stays.
//
// KEEPING THE WALL WITH NO FIELD BEHIND IT IS DELIBERATE. This file's guarantee
// was never "the providerKeyFile box is safe", it was "no param that looks like
// a vendor key is written down", and that claim is about every param a step is
// ever handed. Deleting it because the one field that motivated it is gone
// would trade a working guarantee for a tidier diff -- the same reasoning that
// keeps `vendor_api_key` alive as a globalSecret kind for the router's BYOK
// path and the Shopify connector, which are credentials for vendors MemQL does
// not federate with.
//
// Deliberately free of `vscode` and of `node:fs`: it is a string predicate, and
// its callers are modules that must stay unit-testable without either.
//
// Refs: #3545 #3544 memql#5088

/**
 * What a redacted param reads as in the receipt.
 *
 * A marker rather than an empty string, and rather than dropping the value
 * outright: "" reads as "this param was never given", and a param that WAS
 * given and was refused is a different fact. The operator reading this receipt
 * six weeks later deserves to see which one happened.
 */
export const REDACTED = "[redacted: a provider key was given where a path was expected]";

/**
 * Whether a value is a vendor API key rather than an ordinary param.
 *
 * MATCHED ON THE VENDOR PREFIX, not on shape. The vendors this product calls
 ...
 *
 * It is not a general secret detector and does not pretend to be; `withholdResult`
 * below is the wider gate, for the different population that step RESULTS are.
 *
